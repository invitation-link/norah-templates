...
export default function AnalyticsConsent() {
  ...
  const choose = (granted: boolean) => {
    ...
    if (granted) window.gtag?.("event", "page_view", { page_title: document.title, page_location: window.location.href, page_path: `${window.location.pathname}${window.location.search}` });
    window.dispatchEvent(new Event(EVENT));
  };

  return (
    <aside className={styles.banner} aria-label="Analytics preferences">
      <div><strong>Help us improve Invite Link</strong><p>Allow anonymous usage analytics and session insights. We do not use advertising cookies. <Link href="/privacy">Privacy</Link></p></div>
