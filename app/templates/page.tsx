  ...
  return <main className={styles.page}>
    ...
    <section className={styles.collection}>
      ...
      <div className={styles.grid}>{templates.map((template) => <article key={template.id}>
        ...
        <div className={styles.meta}><div><span>{template.occasionLabel}</span><h2>{template.name}</h2><p>{template.description}</p><small>{template.interaction}</small></div>{template.editorUrl ? <a href={template.editorUrl} target={template.editorUrl.startsWith("http") ? "_blank" : undefined} rel={template.editorUrl.startsWith("http") ? "noreferrer" : undefined}>Customize <ArrowRight size={17} /></a> : <a href={template.liveUrl} target="_blank" rel="noreferrer">Preview <ExternalLink size={16} /></a>}</div>
      </article>)}</div>
    </section>
