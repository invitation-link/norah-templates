...
export default async function Image({ params }: { params: Promise<{ shareId: string }> }) {
  ...
  return new ImageResponse(
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "64px 72px", color: "white", background: "linear-gradient(155deg,#061d30,#0b405c 60%,#b25e3d)", fontFamily: "Arial" }}>
      <span style={{ color: "#F2CC69", fontSize: 20, letterSpacing: 4 }}>PASS THE TIRANGA · INVITE LINK</span>
      <div style={{ display: "flex", flexDirection: "column" }}><span style={{ fontSize: 76, lineHeight: 1, letterSpacing: -4 }}>{inviter} passed the<br />Tiranga to you.</span><span style={{ marginTop: 28, fontSize: 28, color: "rgba(255,255,255,.75)" }}>A Tiranga is waiting. Hoist yours →</span></div>
