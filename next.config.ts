...
const nextConfig: NextConfig = {
  ...
  compress: true,
  async headers() {
    const privateNoIndexHeaders = [{ key: "X-Robots-Tag", value: "noindex, nofollow, noarchive, noimageindex" }];
    const previewNoIndexHeaders = [{ key: "X-Robots-Tag", value: "noindex, follow, noarchive" }];
    return [
      { source: "/:path*", headers: [
      ...
      ...["/i/:path*", "/p/:path*", "/dashboard/:path*", "/editor/:path*", "/growth/:path*", "/_create_legacy/:path*", "/api/:path*"].map((source) => ({ source, headers: privateNoIndexHeaders })),
      ...["/demo/:path*", "/u/:path*"].map((source) => ({ source, headers: previewNoIndexHeaders })),
    ];
  },
};
